...
class ProjectCog(commands.Cog):
    ...
    # ------------------------------------------------------------------
    # 인프라 생성 로직 (핵심 수정: 포럼 생성 실패 시 텍스트 채널로 대체)
    # ------------------------------------------------------------------
    async def _create_project_infrastructure(self, guild, name):
        ...
        try:
            ...
            # 3. 포럼 채널 생성 시도 (안전장치 추가)
            forum_id = None
            forum_mention = ""
            
            try:
                # 라이브러리가 ForumTag를 지원하는지 확인
                if hasattr(discord, 'ForumTag') and hasattr(category, 'create_forum_channel'):
                    forum_tags = [
                        discord.ForumTag(name="TODO", emoji="⬜"),
                        discord.ForumTag(name="IN_PROGRESS", emoji="🔵"),
                        discord.ForumTag(name="DONE", emoji="✅")
                    ]
                    forum = await category.create_forum_channel(
                        name="📌 이슈-보드",
                    ...
                    raise AttributeError("Forum features not supported")
            except (AttributeError, discord.HTTPException) as e:
                # 포럼 생성 실패 시 (버전 문제 or 커뮤니티 기능 미활성) -> 텍스트 채널로 대체
                print(f"[WARNING] 포럼 생성 실패: {e}. 텍스트 채널로 대체합니다.")
                backup_channel = await category.create_text_channel(
                    name="📌 이슈-보드-텍스트",
                    topic=f"[{name}] 프로젝트 이슈 관리 (포럼 미지원으로 대체됨)"
                )
                await backup_channel.send("ℹ️ **알림**: 이 서버 환경에서는 포럼 채널을 생성할 수 없어 텍스트 채널로 대체되었습니다.")
                forum_id = backup_channel.id
                forum_mention = backup_channel.mention

            # 4. 회의록 채널 생성 (텍스트 채널)
            meeting_ch = await category.create_text_channel(
                name="🎙️ 회의록",
                topic=f"[{name}] 회의 기록 아카이브"
            )
            ...
            pid = self.bot.db.create_project(
                ...
                meeting_channel_id=meeting_ch.id
            )
            
            if pid:
                return True, f"✅ **{name}** 프로젝트 공간 생성 완료!\n- 카테고리: {category.name}\n- 이슈보드: {forum_mention}\n- 회의록: {meeting_ch.mention}"
            else:
                # DB 등록 실패 시 롤백
                try: await category.delete() # 카테고리 삭제 시 하위 채널은 남을 수 있으니 주의 (수동 삭제 필요할 수 있음)
                except: pass
                return False, "❌ DB 등록 중 오류가 발생했습니다."

        except discord.Forbidden:
            return False, "❌ 봇에게 '채널/카테고리 관리' 권한이 없습니다. (서버 설정 > 역할 에서 권한을 확인하세요)"
    ...
    async def refresh_dashboard(self, guild_id):
        ...
        for t in ts:
            ...
            link_md = ""
            if len(t) > 6 and t[6]: # thread_id 확인
                # 포럼 스레드면 링크 추가 (간단히 아이콘으로)
                link_md = " 🔗"
    ...
    async def add_task(self, ctx, project: str = None, *, content: str = None):
        if content:
            ...
            if project_data and project_data.get('forum_channel_id'):
                ...
                # 포럼 채널인지 확인 (일반 텍스트 채널일 수도 있음 - 안전장치)
                if forum:
                    try:
                        # ForumChannel인 경우
